	...
	class Install {
		...
		public function ifHag5() {
			...
			if (empty($user) or empty($password)) {
				return "<p>You must to fill all gaps</p>";
			}
		...
		public function ifHag4() {
			...
			if (empty($play_list_file) or empty($cf_ezstream) or empty($cf_icecast)) {
				return "<p>Not all the gaps are filled.</p>";
			}
            
            if (!file_exists($cf_icecast)){
                return"<p>Icecast configuration file doesn't exist.</p>";
            }
            
            if (!file_exists($cf_ezstream)) {
                return "<p>Ezstream configuration file doesn't exist.</p>";
            }
            
			if (!file_exists($play_list_file)) {
				return "<p>Playlist file doesn't exist.</p>";
			}
		...
		public function ifHag3() {
			...
			if(!@ssh2_auth_password($con, $this->request->getPostVar('ssh_user'), $this->request->getPostVar('ssh_pass'))) {
				return "<p>Login or password is not valid.</p>";
    		}
		...
		public function ifHag2() {
			...
			if ($link and $link_db) {
				...
				return "<p>Connection couldn't be established.</p>";
			}
		}

		public function createTable($db_name) {
			mysql_query(include_once('install.sql'))
			 or die("Install query failed : " . mysql_error());
			 			 
             $this->saveSetting('main_text', 'You may keep here some texts.');
             $this->saveSetting('online', '0');
		}

		public function getPerms($file) {
        	if (is_writable($file)) {
        		return '<span class="green"><b>writable</b></span>';
        	} else {
		...
		public function getSsh2() {
			if (function_exists("ssh2_connect")) {
				...
				return '<span class="red"><b>is not installed</b></span>';
			}
		}
